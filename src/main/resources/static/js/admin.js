  ...
  try {
    ...
    if (badgeEl) {
      badgeEl.className =
        ...
        (newState ? "badge-available" : "badge-soldout");
...
async function loadMenuForEdit(menuId, form) {
  ...
  try {
    ...
    if (previewEl && menu.imageUrl) {
      previewEl.innerHTML = `<img src="${menu.imageUrl}" alt="미리보기">`;
    } else {
      previewEl.innerHTML = `<img src="/images/menu/empty.png" alt="미리보기">`;
    }
  } catch (e) {
...
function renderOrderDetailHtml(data) {
  ...
  return `
    <div class="form-group order-detail-summary" style="margin-bottom: 18px;">
      ...
      <div class="order-detail-row" style="border-bottom:1px solid var(--border); padding-bottom:12px;"><span class="order-detail-label">할인</span><span>${data.discountAmount > 0 ? "-" + formatWon(data.discountAmount) : "-"}</span></div>
    </div>
